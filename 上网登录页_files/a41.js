...
var rebackLink="http://10.3.38.8/a79.htm?isReback=1";//返回重定向
...
var regURL="http://192.168.32.185:8080";// 短信自服务地址，如:http://cloud.doctorcom.com/CloudSelf/

var apiBase="https://wep.cau.edu.cn/"; // 中国农业大学定制
...
var page = {
		...
		util._jsonp({
			...
			success: function(json) {
				...
				if (json.result == 1) {
					...
					term.account = term.account.replace(this.tempAccountSuffix,"");
					term.online = json;
					me.kind = term.type == 2 ? 'mobile_31': 'pc_1';
	...
	load_js_css: function(callback) {
		...
		util._load('js','a43.js',function(){
			// util._load('js',  apiBase + 'js/qrcode.js');  // 中国农业大学-XQ2019121308455
			// util._load('js',  apiBase + 'js/mxsocket.js');  // 中国农业大学-XQ2019121308455
			// 上面两个 js 文件，需要访问外网才能取到，未认证时可能取不到，所以改为本地文件
			util._load('js',  'a61.js'); // qrcode.js
			util._load('js',  'a62.js'); // mxsocket.js
			...
			util._load('js',  'a46.js');  // myChart
			util._load('js',  'a48.js');  // bootstrap js v3.2.0
